...
import { Plus, Trash2, CheckCircle2, Circle, RotateCcw, CalendarDays, Copy, Check, Send } from 'lucide-react'
...
export default function Tareas({ esStaff, personas, tareas }: { esStaff: boolean; personas: Persona[]; tareas: Tarea[] }) {
  ...
  // Filtros: si soy staff añado "Mías" y "Asignadas" para separar; si no, solo estado
  ...
  return (
    <div className="space-y-6">
      {/* Formulario (solo staff) — asigna a otras personas */}
      {esStaff && (
      ...
      {lista.length === 0 ? (
        ...
        <div className="space-y-2">
          {lista.map((t) => {
            ...
            const delegada = t.asignada_por_mi && !t.mia // yo la asigné a otro
            return (
              <div key={t.id} className="flex items-start gap-3 rounded-xl border px-4 py-3"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-1 text-[11px]" style={{ color: '#6B6B7E' }}>
                    {delegada && (
                      <span className="inline-flex items-center gap-1" style={{ color: hecha ? '#22C55E' : '#EAB308' }}>
                        <Send size={10} /> para {t.asignado_nombre ?? '—'} · {hecha ? 'completada' : 'pendiente'}
                      </span>
                    )}
                    ...
                    {hecha && t.completada_at && <span style={{ color: '#22C55E' }}>✓ {new Date(t.completada_at).toLocaleDateString('es-ES', { day: '2-digit', month: 'short' })}</span>}
                  </div>
                </div>
