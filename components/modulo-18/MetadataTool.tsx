  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border p-5" style={{ backgroundColor: '#13131A', borderColor: '#1E1E2E' }}>
        {/* Archivo */}
        <label className="text-xs block mb-1" style={{ color: '#6B6B80' }}>Archivo (imagen o video)</label>
        {archivo ? (
          <div className="flex items-center gap-2 rounded-lg px-3 py-2.5 text-sm mb-4" style={inputStyle}>
            <span className="truncate flex-1" style={{ color: '#F0F0F5' }}>{archivo.name}</span>
            <span className="text-xs" style={{ color: '#6B6B80' }}>{(archivo.size / 1024 / 1024).toFixed(1)} MB</span>
          ...
          <label className="flex items-center justify-center gap-2 rounded-lg px-3 py-6 text-sm cursor-pointer mb-4 border border-dashed"
            style={{ backgroundColor: '#0D0D14', borderColor: '#1E1E2E', color: '#8B8B9E' }}>
        ...
        {/* Nº de versiones */}
        <label className="text-xs block mb-1" style={{ color: '#6B6B80' }}>¿Cuántas versiones? (1–12)</label>
        <div className="flex items-center gap-3 mb-4">
          <input type="range" min={1} max={12} value={versiones}
            onChange={(e) => setVersiones(Number(e.target.value))} className="flex-1" />
          <span className="text-lg font-bold w-8 text-center" style={{ color: '#C9A84C' }}>{versiones}</span>
        </div>
          ...
          <div className="divide-y" style={{ borderColor: '#1E1E2E' }}>
            {variantes.map((v, i) => (
              <div key={i} className="flex items-center justify-between px-4 py-3">
                <span className="text-sm truncate" style={{ color: '#F0F0F5' }}>Versión {i + 1} · {v.filename}</span>
                <a href={v.url} target="_blank" rel="noopener noreferrer" download
                  ...
                  style={{ backgroundColor: '#1E1E2E', color: '#C9A84C' }}>
