      ...
      <div className="w-full max-w-md rounded-2xl p-5" style={{ backgroundColor: 'var(--surface)', border: '1px solid var(--border)' }} onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold" style={{ color: 'var(--foreground)' }}>Referencia de…</h3>
          </div>
          <button onClick={onClose} style={{ color: 'var(--muted)' }} className="p-1 rounded hover:bg-[var(--hover)]"><X size={16} /></button>
        </div>
        <p className="text-xs mb-4" style={{ color: 'var(--muted)' }}>Marca a qué modelos sirve <span style={{ color: 'var(--foreground)' }}>{nombreCompetidor}</span> como referencia. Puede ser varias.</p>

        {cargando ? (
          <div className="flex items-center justify-center py-10"><Loader2 size={20} className="animate-spin" style={{ color: 'var(--muted)' }} /></div>
        ) : modelos.length === 0 ? (
          <p className="text-sm py-6 text-center" style={{ color: 'var(--muted)' }}>No hay modelos para elegir.</p>
        ) : (
        ...
        <div className="flex items-center justify-between">
          <span className="text-xs" style={{ color: 'var(--muted)' }}>{seleccion.size} seleccionada{seleccion.size === 1 ? '' : 's'}</span>
          <button onClick={guardar} disabled={guardando || cargando} className="flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-xs font-medium transition-all disabled:opacity-50" style={{ backgroundColor: 'var(--gold-15)', border: '1px solid rgba(201,168,76,0.35)', color: 'var(--gold)' }}>
