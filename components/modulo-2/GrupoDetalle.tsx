  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="w-12 h-12 rounded-full overflow-hidden flex-shrink-0" style={{ border: '1.5px solid var(--gold-25)', backgroundColor: 'var(--border)' }}>
          {grupo.profilePic ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={grupo.profilePic} alt={grupo.nombre} className="w-full h-full object-cover" />
          ) : (
            <div className="w-full h-full flex items-center justify-center text-lg font-bold" style={{ color: 'var(--gold)' }}>{grupo.nombre[0]?.toUpperCase()}</div>
          )}
        </div>
        <div>
