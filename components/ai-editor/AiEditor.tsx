...
import ColaAirtable from './ColaAirtable'

export default function AiEditor() {
  const [tab, setTab] = useState<'videos' | 'cola' | 'perfiles'>('videos')
  const label: Record<string, string> = { videos: 'Vídeos', cola: 'Cola Airtable', perfiles: 'Editing Profiles' }
  return (
    <div className="space-y-5">
      <div className="flex gap-2 flex-wrap">
        {(['videos', 'cola', 'perfiles'] as const).map((t) => (
      ...
      {tab === 'videos' ? <ListaJobs /> : tab === 'cola' ? <ColaAirtable /> : <PerfilesEdicion />}
    </div>
  )
}
