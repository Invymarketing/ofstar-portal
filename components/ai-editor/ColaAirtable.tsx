    ...
    <div className="space-y-3">
      ...
      {tasks.length === 0 ? (
        ...
        <div className="rounded-2xl overflow-hidden" style={card}>
          {tasks.map((t, i) => (
            <div key={t.recordId + t.dNum} className="flex items-center gap-3 px-4 py-3" style={{ borderTop: i === 0 ? 'none' : '1px solid var(--border)' }}>
              ...
              <span className="text-[11px] font-semibold rounded-full px-2.5 py-1 flex-shrink-0" style={{ backgroundColor: 'var(--background)', color: urgColor(t.urgencia), border: '1px solid var(--border)' }}>
                {t.urgencia}
              </span>
            </div>
          ))}
      ...
      <p className="text-[11px]" style={{ color: 'var(--muted)' }}>
        Esto es solo lectura: Skeilab está leyendo tu Airtable. En la siguiente fase añadiremos el botón para editar cada uno.
      </p>
    </div>
  )
}
